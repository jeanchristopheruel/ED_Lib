            ...
            if (arcs[i].isEllipse) {
                ...
                if (arcs[i].coverRatio >= CANDIDATE_ELLIPSE_RATIO && arcs[i].ellipseFitError <= ELLIPSE_ERROR) {
                    addCircle(circles1, noCircles1, arcs[i].xc, arcs[i].yc, arcs[i].r, arcs[i].circleFitError,
                              &arcs[i].eq, arcs[i].ellipseFitError,
                    ...
                    if (circleFitError <= MYERROR) {
                        ...
                        if ((coverage >= FULL_CIRCLE_RATIO && circleFitError <= LONG_ARC_ERROR)) {
                            ...
                            addArc(edarcs1->arcs, edarcs1->noArcs, xc, yc, radius, circleFitError, sTheta, eTheta,
                                   ...
                                   (int) x[0], (int) y[0], (int) x[noPixels - 1], (int) y[noPixels - 1], x, y,
                                   noPixels);
                        } //end-else
                    ...
                    if (isAlmostClosedLoop || (iter == 1 && coverage >= 0.25)) {  // an arc covering at least 90 degrees
                        ...
                        if (ellipseFitError <= MYERROR) {
                            ...
                            if (isAlmostClosedLoop) {
                                addCircle(circles1, noCircles1, xc, yc, radius, circleFitError, &eq, ellipseFitError, x,
                                          y, noPixels);  // Add an ellipse for validation
...
//
    void
    EDCircles::addArc(MyArc *arcs, int &noArcs, double xc, double yc, double r, double circleFitError, double sTheta,
...
//
    void
    EDCircles::ComputeStartAndEndAngles(double xc, double yc, double r, double *x, double *y, int len, double *psTheta,
                                        double *peTheta) {
        double sx = x[0];
        double sy = y[0];
        double ex = x[len - 1];
