#include "ED_Lib/EDLines.h"
#include "ED_Lib/EDColor.h"
#include "ED_Lib/NFA.h"
...
//
    void
    EDLines::ComputeClosestPoint(double x1, double y1, double a, double b, int invert, double &xOut, double &yOut) {
        double x2, y2;
...
#undef POINT_COUNT
        ...
        dist += ComputeMinDistance((shorter->sx + shorter->ex) / 2.0, (shorter->sy + shorter->ey) / 2.0, longer->a,
...
//
    ...
    void
    EDLines::EnumerateRectPoints(double sx, double sy, double ex, double ey, int ptsx[], int ptsy[], int *pNoPoints) {
